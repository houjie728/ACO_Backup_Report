...
while ($a++ < $reportDays) {
	...
	foreach $dbhost(@ACOserver) {  							
		...
		if ($otherTotal == 0) {
				$buffer .="<td style = \"border: 1px solid #C1DAD7; font-size:11px; padding: 6px 6px 6px 12px;\"bgcolor=\"#B6D7A8\">$otherTotal</td>";
		} else {
				$buffer .="<td style = \"border: 1px solid #C1DAD7; font-size:11px; padding: 6px 6px 6px 12px;\">$otherTotal</td>";
		}
